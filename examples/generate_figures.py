...
============================
Self-contained preliminary-results figures for FEMMI.
Does not require SMPy; Kaiser-Squires uses the built-in FFT implementation.

Outputs (saved to outputs/):
  fig_reconstruction.png  -- truth / FEMMI-MAP / MAP residual / KS-FFT / KS residual
                             Gaussian lens, 10% noise, single realisation seed=42
  fig_masked.png          -- masked field (r < 0.6): MAP inpaints, KS fails
                             adaptive mesh refinement near mask boundary
  fig_convergence.png     -- P3 forward-operator shear convergence vs mesh spacing
...
def nodes_to_grid(nodes, vals, nx=64):
    ...
    return np.where(np.isfinite(g), g, gn), XX, YY
...
def run_ks(nodes, g1, g2):
    """Built-in FFT Kaiser-Squires (no SMPy required)."""
    return kaiser_squires(g1, g2, nodes)


def l2_rel(pred, truth):
    d = np.linalg.norm(truth)
    return float(np.linalg.norm(np.nan_to_num(pred) - truth) / (d if d > 0 else 1.0))
...
kt_grid, XX, YY  = nodes_to_grid(nodes, kappa_true)
...
km_grid, _, _ = nodes_to_grid(nodes, km)
ks_nodes      = run_ks(nodes, g1_obs, g2_obs)
ks_grid, _, _ = nodes_to_grid(nodes, ks_nodes)

e_map = l2_rel(km_grid, kt_grid)
e_ks  = l2_rel(ks_grid, kt_grid)
improv = (e_ks - e_map) / e_ks * 100
print(f"  FEMMI-MAP L2 = {e_map:.3f}")
print(f"  KS-FFT    L2 = {e_ks:.3f}")
...
panel(axes[2], r_map,           "MAP residual",
      cmap="RdBu_r", vmin=-r_max, vmax=r_max)
panel(axes[3], ks_grid,         f"KS-FFT  L2={e_ks:.3f}",   vmin=0, vmax=vmax_k)
r_ks  = ks_grid - kt_grid
r_kmax = float(np.percentile(np.abs(r_ks), 99))
panel(axes[4], r_ks,            "KS residual",
      cmap="RdBu_r", vmin=-r_kmax, vmax=r_kmax)

fig.suptitle(
    r"FEMMI vs Kaiser-Squires  |  Gaussian $\kappa$ ($\sigma=0.5$), 10% noise, seed=42  "
    r"|  $20\times20$ P3 mesh, $\lambda^*$ via Morozov discrepancy principle",
...
km_ag, _, _ = nodes_to_grid(nodes_a, km_a)
kt_ag, _, _ = nodes_to_grid(nodes_a, kappa_a)
ks_a_nodes  = run_ks(nodes_a, g1a_obs, g2a_obs)
ks_ag, _, _ = nodes_to_grid(nodes_a, ks_a_nodes)

e_map_a = l2_rel(km_ag, kt_ag)
e_ks_a  = l2_rel(ks_ag, kt_ag)
improv_a = (e_ks_a - e_map_a) / e_ks_a * 100
print(f"  FEMMI-MAP L2 = {e_map_a:.3f}")
print(f"  KS-FFT    L2 = {e_ks_a:.3f}")
...
panel(axes[3], ks_ag,  f"KS-FFT  L2={e_ks_a:.3f}",   vmin=0, vmax=vmax_a)
rka  = ks_ag - kt_ag
rmka = float(np.percentile(np.abs(rka), 99))
panel(axes[4], rka,    "KS residual", cmap="RdBu_r", vmin=-rmka, vmax=rmka)

fig.suptitle(
    rf"Masked reconstruction ($r < {MASK_R}$)  |  "
    r"Matérn prior inpaints via $\kappa$ correlation structure  "
...
# Comparison is restricted to deep interior nodes (|x|,|y| < 1.5) to exclude
# boundary nodes where the shear operator is deliberately zeroed.
#
